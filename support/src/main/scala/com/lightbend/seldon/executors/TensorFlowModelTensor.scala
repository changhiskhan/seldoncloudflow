 ...
 */
class TensorFlowBundleModel(descriptor: ModelDescriptor, directory: String)
...
object TensorFlowBundleModel {
  def apply(descriptor: ModelDescriptor, directory: String): Option[TensorFlowBundleModel] = {
    try {
      Some(new TensorFlowBundleModel(descriptor, directory))
    } catch {
      case t: Throwable ⇒
        println(s"Failed to create TensorFlowBundleModel, descriptor $descriptor; directory $directory")
        t.printStackTrace()
        None
    }
  }
